...
if [ $rc = 0 ] ; then
  ...
  scp -P 20003 ${TMPDIR}/${STATIC_ZIPNAME} ${REMOTE_SSHURI}:${REMOTE_STATIC_PATH}
  ssh -p 20003 ${REMOTE_SSHURI} "unzip -oq ${REMOTE_STATIC_PATH}/${STATIC_ZIPNAME} -d ${REMOTE_STATIC_PATH}"
  scp -P 20004 ${TMPDIR}/${STATIC_ZIPNAME} ${REMOTE_SSHURI}:${REMOTE_STATIC_PATH}
  ssh -p 20004 ${REMOTE_SSHURI} "unzip -oq ${REMOTE_STATIC_PATH}/${STATIC_ZIPNAME} -d ${REMOTE_STATIC_PATH}"
  ...
  scp -P 20003 ${TMPDIR}/${BUILD_ZIPNAME} ${REMOTE_SSHURI}:${REMOTE_BUILD_PATH}
  ssh -p 20003 ${REMOTE_SSHURI} "unzip -oq ${REMOTE_BUILD_PATH}/${BUILD_ZIPNAME} -d ${REMOTE_BUILD_PATH}"
  scp -P 20004 ${TMPDIR}/${BUILD_ZIPNAME} ${REMOTE_SSHURI}:${REMOTE_BUILD_PATH}
  ssh -p 20004 ${REMOTE_SSHURI} "unzip -oq ${REMOTE_BUILD_PATH}/${BUILD_ZIPNAME} -d ${REMOTE_BUILD_PATH}"
  echo "publish done."
